...
exports.main = function() {
  ...
  // Make sure our storage object is defined
  if (!require("simple-storage").storage.filteredDomains)
    require("simple-storage").storage.filteredDomains =
      new Array("http://www.example.com/*");
  ...
  cpanel = require("panel").Panel({
    ...
    onShow: function() {cpanel.postMessage(require("simple-storage")
                              .storage.filteredDomains);}
    });
  ...
  widget = require("widget").Widget({
    id: "letgo-widget",
    label: "LetGo",
    content: '<span style="-moz-user-select: none; cursor:default;">LetGo</span>',
    panel: cpanel,
    width: 40
    });
...
function activatePageMod() {
  ...
  pageMod = require("page-mod").PageMod({
    ...
    include: require("simple-storage").storage.filteredDomains.map(domainToFilter),
