...
// We maintain a whitelist of certain subdomains of commonly-blocked domains.
// Users can override this behavior by blocking the subdomain explicitly.
var defaultWhitelist = ["accounts.youtube.com"]; // Gmail sometimes sneakily
                                                 // redirects users without
                                                 // login cookies to youtube,
                                                 // presumably to see if they're
                                                 // logged in there.
var whitelist = [];
...
function activatePageMod() {
  ...
  whitelist = [];
  for each (var item in defaultWhitelist)
    if (ss.storage.filteredDomains.indexOf(item) == -1)
      whitelist.push(item);
  ...
  pageMod = require("page-mod").PageMod({
    ...
    onAttach: function(worker) {
      ...
      // Don't bounce whitelisted domains.
      if (isWhitelisted(ourTab.url))
        return;
...
// Check a url against the whitelist.
function isWhitelisted(url) {
  var { MatchPattern } = require("match-pattern");
  for each (item in whitelist) {
    var pattern = new MatchPattern(domainToFilter(item));
    if (pattern.test(url))
      return true;
  }
  return false;
