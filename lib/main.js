...
exports.main = function() {
  ...
  cpanel = require("panel").Panel({
    ...
    contentScriptFile: require("self").data.url("config.js"),
...
function activatePageMod() {
  ...
  var filteredSites = require("simple-storage").storage.filteredSites;
  if (filterdSites.length == 0)
    filteredSites[0] = "http://www.example.com/*";

  try {
  pageMod = require("page-mod").PageMod({

    // For which URLs is the pagemod deployed?
    include: filteredSites,
